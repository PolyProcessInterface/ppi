...
Pour représenter un événement de l'application, nous avons défini donc une interface \verb|AppEvent|,
		...
		\section{Plan de validation}
			...
			Pour chaque fonctionnalité ajoutée nous ajoutons un test et nous nous assurons que les tests précédents fonctionnent toujours, dans un second temps les nouvelles fonctionnalité sont implantés dans des branchs séparer sur GitHub ,la fusion d'une branche avec la branch master donne lieu a de nouveau teste ,ainsi on s'assure que le développement et la fusion d'une autre branche qui aurait été créer en parallel ne perturbe pas.
			\newline
			\newline
