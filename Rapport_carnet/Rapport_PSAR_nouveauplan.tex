	...
	public void init(String[] args) {
		if (infra.getId() == 0) {
			infra.send(new RingMessage(infra.getId(), 1, 0));
				...
				L'idée pricipale du projet est de produire une API générique permettant d'abstraire le support d'exécution au niveau du code métier, c'est-à-dire produire une couche logicielle afin de pouvoir exécuter de façon transparente le même code sur un simulateur (\verb|PeerSim|) et sur une infrastructure réelle (\verb|MPI|).					
				\subsection{Schema UML des classes génériques}
					\vspace{2mm}
					...
					\newpage
					\subsection{Primitives offertes et explications}
				...
				et également adapter une certaines de nos classes pour \verb|PeerSim|, par exemple rendre \verb|NodeProcess| clonable. 
					\subsubsection{Schema UML Peersim et explications}
					\vspace{1.2mm}
					\hspace*{-1.3cm} \includegraphics[width=18cm]{uml/peersim1.png}
					\vspace{1.5mm}
					\hspace*{-3.2cm} \includegraphics[width=21cm]{uml/peersim2.png}
			...
			\subsection{Adapter MPI}
				...
				\subsubsection{Schema UML MPI et explications}
				\vspace{5mm}
				\hspace*{-2.3cm} \includegraphics[width=20cm]{uml/mpi1.png}
