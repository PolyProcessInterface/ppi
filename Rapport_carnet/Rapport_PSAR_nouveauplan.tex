...
Pour représenter un événement de l'application, nous avons défini donc une interface \verb|AppEvent|,
			...
			Nous avons, cependant, dû ajouter une boucle afin d'adapter le fonctionnement de \verb|MPI| à celui de \verb|PeerSim|.
				...
				Le diagramme ci-dessus montre la structure interne de l'implémentation vers MPI, dont le cœur est la Class \verb|MpiInfrastructure|
		...
		\section{Plan de validation}
			...
			Pour chaque fonctionnalité ajoutée nous ajoutons un test et nous nous assurons que les tests précédents fonctionnent toujours , dans un second temps les nouvelle fonctionaliter sont implanter dans des branch séparer sur GitHub 
			lors de la fusion d'une  branche avec la branch master GitHub repasse les teste ainsi ont s'assure que le développement et la mise d'une autre branche qui aurais était créer en parallel ne perturbe pas le fonctionnement l'une de l'autre.
			\newline
			\newline
			...
			Nous avons implanter l'algorithme de Naimi-Trehel afin de tester notre implementation du de la fonctionaliter wait , ainsi que celui de l'anneaux pour touts les autres test.
			\newline
			...
			Afin de tester les pannes ainsi que les evennements nous avons utiliser le teste suivant qui mets en panne un processus ainsi il ignoreras touts les message recus saufe Applicatif
			et un autre noeud est mis hors service mais touts de suite remis ont ligne.
			\begin{lstlisting}
