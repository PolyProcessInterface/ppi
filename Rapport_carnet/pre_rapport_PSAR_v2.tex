...
public class BasicTest {
	...
	public void PeersimExample() {
		...
		\section{Les étapes de réalisation}
			...
			L'Api MPI (Message Passing Interface) est avant toute une norme pour le passage de message entre différent ordinateur ou sur le meme ordinateur.
			elle énormement utiliser pour la comunication sur des architecture distribuer.
			Dans notre cas elle c'est réveler extrement interésante car MPI a été implémentée sur presque toutes les architectures de mémoires ainsi elle 
			a était addapter pour chaque une de la façon la plus optimal possible cependant les teste et le debug sur celle-ci reste dificile du a l'imposibiliter
			d'avoir un controle sur les évenements qui apparaisse contrairement a PeerSim.
			\subsection{Implantation vers PeerSim}
			PeerSim est utiliser pour créer des noeud et simuler une architecture pair-à-pair ont générant des protocol et des évenements
			qui sont définis par l'utilisateur , l'éxecution et la suite d'évenements est toujour la meme ce qui permet d'acceler 
			le débug d'une application répartie avant sont déploiament ou de reproduire une séquences d'évenements qui a provoquer un bug
			sur une application déja éxistante, cependant écrire du code ont PeerSim est exhaustif et peux dificilement etre exploiter pour une utilisation
			autre que le debug.
		...
		Pour chaque fonctionnaliter ajouter nous ajoutant un teste et nous nous assurons que les teste précédent fonctionne toujour.
		\newline
		La class BasicTest est la toute 1er classe de teste que nous avons implémenter elle s'assure que les fonctionnaliter de base de notre Api
		send recive et le format de description de scenarion est fonctionnel
